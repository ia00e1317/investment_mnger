...
<div id="list_form">
	...
	<dl>
		<?php foreach ($investorList as $key => $val) { ?>
			<dt><?= h($val) ?></dt>
			<dd>
				<form action="16_investor_delete.php" method="post">
					<input type="hidden" name="investor_no" value="<?= h($key) ?>">
					<input type="button" id="form_submit_button" name="form_submit_button" value="削除" onclick="this.form.submit();"/>
				</form>
			</dd>
		<?php } ?>
	</dl>
	...
	<p id="form_submit" class="right">
		<input id="form_cancel_button" type="button" value="投資家管理メニューへ戻る" onClick="location.href='11_investor_menu.php'">
	</p>
	<p id="form_submit" class="right" >
