...
try {
	...
	if (isset ( $_POST ['form_submit_button'] )) {
		...
		if (count($error) < 1) {
			...
			$last_id = insertProject($pdo, $info);		// 投資プロジェクト登録

			// 配当
			for($i = 1 ; $i < count($_POST['haito_dividend_date']); $i++) {

				$haito = new payment_def();
				$haito->setProjectNo($last_id);
				$haito->setType('01');
			...
			// 「投資案件管理メニュー」画面へ移動
			header('Location: 21_project_menu.php');
			exit();
		}
	...
	// 初期表示
		...
		$info = new project();
		$info->setDividendMonth('1');
		$info->setDividendDate('15');
...
<form action="" method="post" id="mail_form" name="mail_form">
	...
	<dl>
		...
		<dd>
			<ul>
				<li><label> <input type="radio" class="investment_type" name="investment_type" value="01"
					<?php if (strcmp($info->getType(), '01') == 0) { echo 'checked'; } ?> /> 再生
				</label></li>
				<li><label> <input type="radio" class="investment_type" name="investment_type" value="02"
					<?php if (strcmp($info->getType(), '02') == 0) { echo 'checked'; } ?> /> 転売
				</label></li>
			</ul>
		</dd>
		...
		<dl>
			...
			<dd>
				...
				<input type="number" id="dividend_date" name="dividend_date" value="<?= h($info->getDividendDate()) ?>"
					min="0" max="31" step="1" required="required" style="width: 13%;">&nbsp;日
				<input type="number" id="dividend_count" name="dividend_count" value="<?= h($info->getDividendCount()) ?>"
					min="0" max="" step="1" required="required" style="width: 13%;"> &nbsp;回
			</dd>
	...
	<div id="haitoDetail" style="display: none;">
		<h2>配当日</h2>
		<dl>
