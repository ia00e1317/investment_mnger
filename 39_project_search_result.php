...
try {
	...
	if (isset($_POST['funcId'])) {
		...
		if (strcmp($_POST['funcId'], '1') == 0) {
			updateActualPayment($pdo, $_POST['transNo'], $_POST['payment_date'], $_POST['payment_amount']);

		// 取消
		} else if (strcmp($_POST['funcId'], '2') == 0) {
			updateActualPayment($pdo, $_POST['transNo'], null, null);
		}
	...
	// 初期表示時
		...
		$_SESSION['project_date'] = $_POST['project_date'];
	}

	// 検索条件
	$project = getProjectInfo($pdo, $_SESSION['project_no']);

	// 支払予実取得
	$paymentList = getPaymentListByProject($pdo, $_SESSION['project_no'], $_SESSION['project_date']);
...
<!-- <form action="" method="post" id="mail_form" name="mainForm"> -->
	<h1>投資案件検索結果</h1>
	<dl>
		<dt>案件名：(<?= h($project->getTypeName()) ?>)<?= h($project->getName()) ?></dt>
		<dd>
			対象年月：<?= date('Y年n月', strtotime($_SESSION['project_date'])); ?>
		</dd>

		<?php foreach ($paymentList as $value) { ?>
		<dt>投資家名：<?= h($value['investorName']) ?><br><?= h($typeLabel[$value['paymentType']]) ?></dt>
		<dd>
			<form action="" method="post">
				...
				<ul id="thanks2">
					<li> 支払予定額：</li>
					<li> <?php echo number_format($value['plannedAmount']); ?> 円</li>
				</ul>
				<ul id="thanks2">
					<li> 支払実績額：</li>
					<li>
				...
				<input type="hidden" name="transNo" value="<?= $value['transNo'] ?>">
